...
bool EasyVR::recvArg(int8_t& c)
{
  ...
  c = r - ARG_ZERO;
  return r >= ARG_MINIM && r <= ARG_MAXIM;
}
...
void EasyVR::bridgeLoop(Stream& serial)
{
  ...
  for (;;)
  {
    ...
    if (_s->available())
      serial.write(_s->read());
  }
}

int EasyVR::bridgeRequested(Stream& serial)
{
